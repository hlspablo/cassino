        ...
        <div class="modal-content">
            ...
            <form id="depositForm" method="post" action="" class="form-login">
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="number" name="amount" required min="<?php echo e(config('setting')->min_deposit); ?>" max="<?php echo e(config('setting')->max_deposit); ?>" class="form-control" placeholder="0,00">
                </div>
                <?php if(!empty(auth()->user()) && !auth()->user()->is_demo_agent): ?>
                    <div class="input-group mb-3">
                ...
                <div class="row justify-between list-amount">
                    <?php $amount = config('setting')->min_deposit ?>
                    <?php for($i = 0; $i < 6; $i++): ?>
                        <div class="col-sm-4 col-lg-2">
                            <button type="button" class="select_amount">R$ <?php echo e($amount); ?></button>
                        </div>
                        <?php $amount = $amount * 2 ?>
                    <?php endfor; ?>
                </div>
                ...
                <div class="d-grid mt-3">
                    <button type="submit" class="btn-primary-theme btn-block w-full mb-3">
                        <?php if(!empty(auth()->user()) && !auth()->user()->is_demo_agent): ?> GERAR QRCODE <?php else: ?> ENVIAR <?php endif; ?>
                    </button>
                </div>
            </form>
        </div>
...
<?php $__env->startPush('scripts'); ?>
    ...
    <script>
        ...
        {
            fetch('<?php echo e(url('suitpay/consult-status-transaction')); ?>', {
                method: 'POST',
                body: JSON.stringify({
                ...
                .then(data => {
                    if(data.status == 'PAID') {
                        ...
                        setTimeout(function() {
                            window.location.replace('<?php echo e(route('panel.wallet.deposits')); ?>');
                        }, 500);
                    }
                })
                .catch(error => {
        ...
        document.getElementById('depositForm').addEventListener('submit', function(event) {
            ...
            fetch('<?php echo e(url('suitpay/qrcode-pix')); ?>', {
                method: 'POST',
                body: formData
            })
                .then(response => response.json())
...
<?php $__env->stopPush(); ?>
<?php /**PATH /Users/hlspablo/code/cassino/resources/views/includes/deposit.blade.php ENDPATH**/ ?>